...
Split apps/api/src/services/key-rotation.service.ts (658 LOC) into apps/api/src/services/key-rotation/ with per-verb files, each ≤300 LOC. Follows the pattern established by api-trlq (member.service.ts refactor).

## Context

Currently concentrates rotation state machine / envelope re-wrapping / ledger writes in a single file. Splitting along verbs (create / update / archive / queries / permissions) makes the unit of change smaller and the domain-local permissions logic discoverable.

## Scope

- [ ] Create apps/api/src/services/key-rotation/ directory
...
- apps/api/src/services/key-rotation.service.ts no longer exists
- pnpm vitest run --project api passes
- pnpm vitest run --project api-integration passes
- pnpm typecheck passes
- No file in apps/api/src/services/key-rotation/ exceeds 300 LOC
