...
- `notification_configs`: id (UUID PK), system_id (FK → systems, NOT NULL), event_type (varchar, T3, NOT NULL — one row per event type), enabled (boolean, T3, NOT NULL, default true), push_enabled (boolean, T3, NOT NULL, default true), created_at (T3, NOT NULL, default NOW()), updated_at (T3)
  - Unique: (system_id, event_type)
- `friend_notification_preferences`: id (UUID PK), system_id (FK → systems, NOT NULL), friend_connection_id (FK → friend_connections, NOT NULL), enabled_event_types (varchar[] or JSON, T3, NOT NULL — array of FriendNotificationEventType), created_at (T3, NOT NULL, default NOW()), updated_at (T3)
