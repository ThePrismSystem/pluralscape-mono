...
- **`audit_log`**: id (UUID PK), account_id (FK → accounts, NOT NULL), event_type (varchar, T3, NOT NULL), timestamp (T3, NOT NULL), ip_address (varchar, T3, nullable), user_agent (varchar, T3, nullable), metadata (T3, NOT NULL — event-specific details)
...
login-success, login-failed, session-created, session-revoked, password-changed, recovery-key-used, api-key-created, api-key-revoked, data-export, account-purge-requested, device-added, device-removed
