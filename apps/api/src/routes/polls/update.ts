...
import { updatePoll } from "../../services/poll.service.js";
...
updateRoute.use("*", requireScopeMiddleware("write:polls"));
...
updateRoute.put("/:pollId", async (c) => {
  ...
  const result = await updatePoll(db, systemId, pollId, body, auth, audit);
