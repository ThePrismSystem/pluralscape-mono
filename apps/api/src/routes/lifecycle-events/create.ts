...
import { createLifecycleEvent } from "../../services/lifecycle-event.service.js";
...
createRoute.use("*", requireScopeMiddleware("write:lifecycle-events"));
...
createRoute.post("/", async (c) => {
  ...
  const result = await createLifecycleEvent(db, systemId, body, auth, audit);
