...
listRoute.use("*", requireScopeMiddleware("read:webhooks"));
...
listRoute.get("/", async (c) => {
  ...
  const queryOpts = parseWebhookDeliveryQuery(c.req.query());

  const db = await getDb();
  const result = await listWebhookDeliveries(db, systemId, auth, {
    ...queryOpts,
