...
import { createRegion } from "../../../services/innerworld-region.service.js";
...
createRoute.use("*", requireScopeMiddleware("write:innerworld"));
...
createRoute.post("/", async (c) => {
  ...
  const result = await createRegion(db, systemId, body, auth, audit);
