...
import { createFrontingReport } from "../../services/fronting-report.service.js";
...
createRoute.use("*", requireScopeMiddleware("write:reports"));
...
createRoute.post("/", async (c) => {
  ...
  const result = await createFrontingReport(db, systemId, body, auth, audit);
