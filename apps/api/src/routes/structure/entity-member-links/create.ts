...
import { createEntityMemberLink } from "../../../services/structure/member-link.js";
...
createRoute.post("/", async (c) => {
  ...
  const parsed = CreateStructureEntityMemberLinkBodySchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const result = await createEntityMemberLink(db, systemId, parsed.data, auth, audit);
