...
import { createEntityLink } from "../../../services/structure/link.js";
...
createRoute.post("/", async (c) => {
  ...
  const parsed = CreateStructureEntityLinkBodySchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const result = await createEntityLink(db, systemId, parsed.data, auth, audit);
