...
import { updateEntityLink } from "../../../services/structure-entity.service.js";
...
updateRoute.use("*", requireScopeMiddleware("write:structure"));
...
updateRoute.put("/:linkId", async (c) => {
  ...
  const result = await updateEntityLink(db, systemId, linkId, body, auth, audit);
