...
import { createEntityAssociation } from "../../../services/structure/association.js";
...
createRoute.post("/", async (c) => {
  ...
  const parsed = CreateStructureEntityAssociationBodySchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const result = await createEntityAssociation(db, systemId, parsed.data, auth, audit);
