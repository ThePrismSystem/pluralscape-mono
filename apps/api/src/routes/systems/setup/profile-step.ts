...
import { SetupProfileStepBodySchema } from "@pluralscape/validation";
...
profileStepRoute.post("/", async (c) => {
  ...
  const parsed = SetupProfileStepBodySchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  await setupProfileStep(db, systemId, parsed.data, auth, audit);
