...
listRoute.use("*", requireScopeMiddleware("read:check-ins"));
...
listRoute.get("/", async (c) => {
  ...
  const queryOpts = parseCheckInRecordQuery(c.req.query());

  const db = await getDb();
  const result = await listCheckInRecords(db, systemId, auth, {
    ...queryOpts,
