...
import type { RegistrationCommitSchema, RegistrationInitiateSchema } from "@pluralscape/validation";
...
export async function initiateRegistration(
  ...
  body: z.infer<typeof RegistrationInitiateSchema>,
  retried = false,
  ...
  const encryptedEmailBytes = getEmailEncryptionKey() ? encryptEmail(body.email) : null;
    ...
    if (isDuplicateEmailError(error)) {
      ...
      if (existing) {
        ...
        if (isPlaceholder && isExpired && !retried) {
          ...
          return initiateRegistration(db, body, true);
        }
      }
...
export async function commitRegistration(
  db: PostgresJsDatabase,
  body: z.infer<typeof RegistrationCommitSchema>,
  platform: ClientPlatform,
  audit: AuditWriter,
): Promise<RegistrationCommitResult> {
  if (!body.recoveryKeyBackupConfirmed) {
  ...
  const [account] = await db
    .select()
    .from(accounts)
    .where(eq(accounts.id, brandId<AccountId>(body.accountId)))
    .limit(1);
  ...
  const publicSigningKeyBytes = fromHex(body.publicSigningKey);
  ...
  const authKeyRaw = fromHex(body.authKey);
  ...
  const encryptedMasterKeyBytes = fromHex(body.encryptedMasterKey);
  const encSignPrivKeyBytes = fromHex(body.encryptedSigningPrivateKey);
  const encEncPrivKeyBytes = fromHex(body.encryptedEncryptionPrivateKey);
  const publicEncKeyBytes = fromHex(body.publicEncryptionKey);
  const recoveryEncMasterKeyBytes = fromHex(body.recoveryEncryptedMasterKey);
  ...
  await withAccountTransaction(db, brandId<AccountId>(account.id), async (tx) => {
    ...
    await tx.insert(recoveryKeys).values({
      ...
      recoveryKeyHash: fromHex(body.recoveryKeyHash),
      createdAt: timestamp,
    });
