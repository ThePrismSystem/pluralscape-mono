...
export async function initiateRegistration(
  ...
  try {
    await withAccountTransaction(db, brandId<AccountId>(accountId), async (tx) => {
      // The challenge + email columns are branded `ServerInternal<…>` on the
      // schema — tag the locally generated bytes/timestamps at the insert site
      // so Drizzle's typed `.values({…})` accepts them. Compile-time only.
