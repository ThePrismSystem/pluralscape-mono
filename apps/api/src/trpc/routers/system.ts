...
export const systemRouter = router({
  ...
  get: systemProcedure.query(async ({ ctx }) => {
  ...
  update: systemProcedure.input(UpdateSystemBodySchema).mutation(async ({ ctx, input }) => {
    const audit = ctx.createAudit(ctx.auth);
  ...
  archive: systemProcedure.mutation(async ({ ctx }) => {
    const audit = ctx.createAudit(ctx.auth);
    await archiveSystem(ctx.db, ctx.systemId, ctx.auth, audit);
    return { success: true as const };
  }),

  duplicate: systemProcedure.input(DuplicateSystemBodySchema).mutation(async ({ ctx, input }) => {
    const audit = ctx.createAudit(ctx.auth);
    return duplicateSystem(ctx.db, ctx.systemId, input, ctx.auth, audit);
  }),

  purge: systemProcedure.input(PurgeSystemBodySchema).mutation(async ({ ctx, input }) => {
    const audit = ctx.createAudit(ctx.auth);
