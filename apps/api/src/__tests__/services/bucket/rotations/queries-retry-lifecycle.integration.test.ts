...
describe("bucket/rotations — getRotationProgress / retryRotation / full lifecycle (PGlite integration)", () => {
  let client: PGlite;
  ...
  describe("retryRotation", () => {
    it("resets only failed items and emits disambiguated audit detail", async () => {
      ...
      // makes the audit detail meaningful.
      const bucketId = await insertBucket(db, systemId);
      ...
      // Audit event records the reset count AND the state transition.
  ...
  describe("full lifecycle", () => {
    it("initiate -> claim all -> complete all -> verify completed state", async () => {
      ...
      const completion1 = await completeRotationChunk(
        ...
        bucketId,
        rotation.id,
        { items: chunk1.data.map((item) => ({ itemId: item.id, status: "completed" as const })) },
        auth,
        noopAudit,
      );
      ...
      const completion2 = await completeRotationChunk(
        ...
        bucketId,
        rotation.id,
        { items: chunk2.data.map((item) => ({ itemId: item.id, status: "completed" as const })) },
        auth,
        noopAudit,
      );
    ...
    it("marks rotation as failed when items exhaust max attempts", async () => {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        await completeRotationChunk(
          ...
          bucketId,
          rotation.id,
          { items: claim.data.map((item) => ({ itemId: item.id, status: "failed" as const })) },
          auth,
          noopAudit,
        );
      }
