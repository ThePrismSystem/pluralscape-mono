...
describe("GET /systems/:systemId/buckets/:bucketId", () => {
  beforeEach(() => {
  ...
  it("returns 200 with bucket", async () => {
    vi.mocked(getBucket).mockResolvedValueOnce(MOCK_RESULT);
  ...
  it("passes ids to service", async () => {
    ...
    expect(vi.mocked(getBucket)).toHaveBeenCalledWith({}, SYS_ID, BUCKET_ID, expect.any(Object));
  });
  ...
  it.each([[404, "NOT_FOUND", "Bucket not found"]] as const)(
    "maps service ApiHttpError %i %s to HTTP response",
    async (status, code, message) => {
      vi.mocked(getBucket).mockRejectedValueOnce(new ApiHttpError(status, code, message));
