...
const { addSubsystemMembership, listSubsystemMemberships, removeSubsystemMembership } =
...
const MEMBERSHIP_ID = "subm_550e8400-e29b-41d4-a716-446655440002";
...
const BASE_URL = `/systems/${SYS_ID}/subsystems/${SUB_ID}/memberships`;
...
describe("POST /systems/:id/subsystems/:subsystemId/memberships", () => {
  beforeEach(() => vi.mocked(addSubsystemMembership).mockReset());
  afterEach(() => vi.restoreAllMocks());

  it("returns 201 with new membership and audit written", async () => {
    vi.mocked(addSubsystemMembership).mockResolvedValueOnce(MOCK_MEMBERSHIP);
    ...
    expect(vi.mocked(addSubsystemMembership)).toHaveBeenCalledWith(
      expect.anything(),
      SYS_ID,
      SUB_ID,
      VALID_BODY,
...
describe("DELETE /systems/:id/subsystems/:subsystemId/memberships/:membershipId", () => {
  beforeEach(() => vi.mocked(removeSubsystemMembership).mockReset());
  afterEach(() => vi.restoreAllMocks());

  it("returns 200 with ok: true", async () => {
    vi.mocked(removeSubsystemMembership).mockResolvedValueOnce(undefined);
  ...
  it("returns 404 when membership not found", async () => {
    ...
    vi.mocked(removeSubsystemMembership).mockRejectedValueOnce(
...
describe("GET /systems/:id/subsystems/:subsystemId/memberships", () => {
  beforeEach(() => vi.mocked(listSubsystemMemberships).mockReset());
  afterEach(() => vi.restoreAllMocks());
  ...
  it("returns 200 with paginated list", async () => {
    ...
    vi.mocked(listSubsystemMemberships).mockResolvedValueOnce(page);
  ...
  it("returns 400 for invalid subsystemId param format", async () => {
    const app = createApp();
    const res = await app.request(`/systems/${SYS_ID}/subsystems/not-valid/memberships`);
