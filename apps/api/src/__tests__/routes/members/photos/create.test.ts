...
const { createMemberPhoto } = await import("../../../../services/member-photo.service.js");
...
describe("POST /systems/:systemId/members/:memberId/photos", () => {
  beforeEach(() => {
    vi.mocked(createMemberPhoto).mockReset();
  });
  ...
  it("returns 201 with created photo on success", async () => {
    vi.mocked(createMemberPhoto).mockResolvedValueOnce(PHOTO_RESULT);
  ...
  it("forwards systemId, memberId, body, auth, and audit writer to service", async () => {
    ...
    const app = createApp();
    await postJSON(app, PHOTOS_PATH, VALID_BODY);

    expect(vi.mocked(createMemberPhoto)).toHaveBeenCalledWith(
      expect.anything(),
      SYS_ID,
      MEM_ID,
      VALID_BODY,
  ...
  it("re-throws unexpected errors as 500", async () => {
    vi.mocked(createMemberPhoto).mockRejectedValueOnce(new Error("DB timeout"));
