...
const { getMember } = await import("../../../services/member/queries.js");
...
describe("GET /systems/:systemId/members/:memberId", () => {
  beforeEach(() => {
  ...
  it("forwards systemId, memberId, and auth to service", async () => {
    ...
    const app = createApp();
    await app.request(`/systems/${SYS_ID}/members/${MEM_ID}`);

    expect(vi.mocked(getMember)).toHaveBeenCalledWith(expect.anything(), SYS_ID, MEM_ID, MOCK_AUTH);
  });

  it("returns 404 when member not found", async () => {
    const { ApiHttpError } = await import("../../../lib/api-error.js");
    vi.mocked(getMember).mockRejectedValueOnce(
  ...
  it("returns 400 for invalid member ID format", async () => {
    const app = createApp();
    const res = await app.request(`/systems/${SYS_ID}/members/not-a-valid-id`);
  ...
  it("returns 400 for invalid system ID format", async () => {
    const app = createApp();
    const res = await app.request(`/systems/not-a-valid-id/members/${MEM_ID}`);
  ...
  it("re-throws unexpected errors as 500", async () => {
    vi.mocked(getMember).mockRejectedValueOnce(new Error("DB timeout"));
