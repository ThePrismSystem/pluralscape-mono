...
type AuthLevel = "public" | "protected" | "system";

interface TRPCProcedureInfo {
...
async function discoverTRPCProcedures(): Promise<Map<string, TRPCProcedureInfo>> {
...
function extractTRPCRateLimits(
...
function buildRESTInventory(): RESTRouteInfo[] {
...
async function main(): Promise<void> {
  // Step 1: Discover tRPC procedures via runtime import
  let trpcProcedures = await discoverTRPCProcedures();

  // Step 2: Enrich tRPC procedures with rate limit categories from source
  trpcProcedures = extractTRPCRateLimits(trpcProcedures);

  // Step 3: Discover REST routes via file system walk
  ...
  // Exit code: 0 if only warnings (including idempotency), 1 if hard failures
  if (failures.length > 0) {
    process.exit(1);
  }
}
