...
describe("enum arrays", () => {
  ...
  it("API_KEY_SCOPES matches ApiKeyScope union", () => {
    ...
    expect(API_KEY_SCOPES).toContain("read:audit-log");
    expect(API_KEY_SCOPES).toContain("read-all");
    expect(API_KEY_SCOPES).toContain("write-all");
    expect(API_KEY_SCOPES).toContain("delete-all");
    expect(API_KEY_SCOPES).toContain("full");
  });

  it("AUDIT_EVENT_TYPES matches AuditEventType union", () => {
    expect(AUDIT_EVENT_TYPES).toHaveLength(221);
  ...
  it("WEBHOOK_EVENT_TYPES matches WebhookEventType union", () => {
    expect(WEBHOOK_EVENT_TYPES).toHaveLength(60);
