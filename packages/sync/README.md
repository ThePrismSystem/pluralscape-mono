...
CRDT documents are managed with [Automerge](https://automerge.org/). Each logical entity (system core, fronting, chat, journal, notes, privacy config, bucket projections) maps to a dedicated Automerge document. Full entity coverage, including webhook configs and friend connections, lives in `ENTITY_CRDT_STRATEGIES`. Changes are encrypted client-side using per-document symmetric keys from `@pluralscape/crypto` before submission. The relay stores envelopes; clients decrypt and apply them locally.
