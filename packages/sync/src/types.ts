...
export const TIME_SPLIT_CONFIGS: readonly TimeSplitConfig[] = [
  {
    ...
    splitUnit: "quarter",
    splitThresholdBytes: FRONTING_SPLIT_THRESHOLD_BYTES,
  },
  { documentType: "chat", splitUnit: "month", splitThresholdBytes: CHAT_SPLIT_THRESHOLD_BYTES },
  {
...
export const DOCUMENT_SIZE_LIMITS: Record<SyncDocumentType, number> = {
  "system-core": SYSTEM_CORE_SIZE_LIMIT_BYTES,
...
export const DEFAULT_STORAGE_BUDGET: StorageBudget = {
  maxTotalBytes: DEFAULT_STORAGE_BUDGET_BYTES,
} as const;
...
export type ConflictResolutionStrategy =
  ...
  | "post-merge-endtime-normalize"
  | "post-merge-timer-normalize"
  | "post-merge-bucket-content-tag-drop"
  | "notification-only";
...
export interface PostMergeValidationResult {
  ...
  readonly bucketContentTagDrops: number;
  readonly correctionEnvelopes: readonly Omit<EncryptedChangeEnvelope, "seq">[];
  /** All conflict notifications generated by the validation run. */
  readonly notifications: readonly ConflictNotification[];
  /** Errors from validators that threw during the run. Empty on full success. */
  readonly errors: readonly { validator: string; error: unknown }[];
}
