...
const DOCUMENT_ID = asSyncDocId("doc-relay-001");
...
describe("EncryptedRelay", () => {
  ...
  it("2.4b — different documents have independent seq counters", async () => {
    const c1 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-alpha"), keys, sodium);
    const c2 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-beta"), keys, sodium);
  ...
  it("2.6 — replaces old snapshot with new one", async () => {
    ...
    expect(await relay.getLatestSnapshot(asSyncDocId("doc-unknown"))).toBeNull();
  });
  ...
  describe("LRU eviction", () => {
    it("evicts oldest doc when limit exceeded", async () => {
      ...
      const c1 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
      const c2 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-b"), keys, sodium);
      const c3 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-c"), keys, sodium);
      ...
      const result = await limitedRelay.getEnvelopesSince(asSyncDocId("doc-a"), 0);
      expect(result.envelopes).toHaveLength(0);
      const resultB = await limitedRelay.getEnvelopesSince(asSyncDocId("doc-b"), 0);
      expect(resultB.envelopes).toHaveLength(1);
      const resultC = await limitedRelay.getEnvelopesSince(asSyncDocId("doc-c"), 0);
    ...
    it("calls onEvict callback on eviction", async () => {
      ...
      const c1 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-x"), keys, sodium);
      const c2 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-y"), keys, sodium);
    ...
    it("touch updates access time for LRU ordering", async () => {
      ...
      const c1 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-old"), keys, sodium);
      const c2 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-new"), keys, sodium);
      ...
      now = 3000;
      await limitedRelay.getEnvelopesSince(asSyncDocId("doc-old"), 0);

      // Now add doc-c — should evict doc-new (less recently accessed)
      now = 4000;
      const c3 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-third"), keys, sodium);
      await limitedRelay.submit(c3);
    ...
    it("does not evict when under limit", async () => {
      ...
      const c1 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-1"), keys, sodium);
      const c2 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-2"), keys, sodium);
    ...
    it("does not self-evict the incoming document", async () => {
      ...
      const c3 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
      await limitedRelay.submit(c3);
    ...
    it("evicts oldest non-incoming doc when new doc arrives at capacity", async () => {
      ...
      expect(evicted).toEqual(["doc-a"]);
      const result = await limitedRelay.getEnvelopesSince(asSyncDocId("doc-c"), 0);
      expect(result.envelopes).toHaveLength(1);
    ...
    it("cleans up seq counters on eviction", async () => {
      ...
      const c1 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
      await limitedRelay.submit(c1);
      const resultA1 = await limitedRelay.getEnvelopesSince(asSyncDocId("doc-a"), 0);
      ...
      const c3 = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
      await limitedRelay.submit(c3);
      const resultA2 = await limitedRelay.getEnvelopesSince(asSyncDocId("doc-a"), 0);
  ...
  describe("per-document envelope limit (H1)", () => {
    ...
    it("enforces limits independently per document", async () => {
      ...
      for (let i = 0; i < maxEnvelopes; i++) {
        const envelope = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
        await limitedRelay.submit(envelope);
      }

      // doc-b should still accept envelopes
      const envelopeB = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-b"), keys, sodium);
      expect(await limitedRelay.submit(envelopeB)).toBe(1);

      // doc-a should still be rejected
      const overflowA = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
  ...
  describe("secondary dedup index (M9)", () => {
    it("evicts and resets document state including dedup entries", async () => {
      ...
      // Evict doc-a by submitting to doc-b
      const cb = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-b"), keys, sodium);
    ...
    it("keeps dedup entries for non-evicted documents intact", async () => {
      ...
      const ea = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
      await limitedRelay.submit(ea);
      const eb = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-b"), keys, sodium);
      await limitedRelay.submit(eb);

      // Evict doc-a by submitting to doc-c
      const ec = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-c"), keys, sodium);
    ...
    it("secondary index stays in sync after multiple evictions", async () => {
      ...
      const ca = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
      await limitedRelay.submit(ca);

      const cb = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-b"), keys, sodium);
      await limitedRelay.submit(cb);

      const cc = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-c"), keys, sodium);
      ...
      const newA = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-a"), keys, sodium);
  ...
  describe("cross-document dedup key collision", () => {
    it("does not dedup the same envelope submitted to different documents", async () => {
      const envelope = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-x"), keys, sodium);
      ...
      const envelopeY = { ...envelope, documentId: asSyncDocId("doc-y") };
    ...
    it("cleans up stale dedupByDoc entry on cross-document collision", async () => {
      ...
      const envelopeB = { ...envelope, documentId: asSyncDocId("doc-b") };
      await limitedRelay.submit(envelopeB);

      // Evict doc-a by filling capacity (doc-a, doc-b already exist, add doc-c and doc-d)
      const ec = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-c"), keys, sodium);
      await limitedRelay.submit(ec);
      const ed = encryptChange(sodium.randomBytes(16), asSyncDocId("doc-d"), keys, sodium);
