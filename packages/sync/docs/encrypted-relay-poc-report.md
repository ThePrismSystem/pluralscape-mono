...
- `submitSnapshot(envelope)` → stores (replaces previous snapshot for same document)
- `getLatestSnapshot(documentId)` → returns latest snapshot or null

No per-peer state. No routing logic. No decryption capability.

### 2.5 Session management

`EncryptedSyncSession` wraps an Automerge document with encryption keys and tracks `lastSeenSeq` to know what to fetch from the relay:

- `change(fn)` → mutates doc, encrypts the resulting change, returns envelope for relay submission
- `applyEncryptedChanges(envelopes)` → decrypts and applies changes, updates `lastSeenSeq`
- `createSnapshot(version)` → encrypts `Automerge.save()` output
- `fromSnapshot(envelope, keys, sodium)` → bootstraps a new session from an encrypted snapshot
...
All 24 tests pass. Each test maps to a specific property being validated.

### 3.1 Encryption primitives (9 tests)
...
**What this proves:** The encryption layer correctly wraps and unwraps Automerge data. Tampering, wrong keys, and cross-document replay are all detected and rejected.
...
| 3.6  | Applying the same Automerge change twice is idempotent                  | PASS   |
...
**What this proves:** The full pipeline works — Automerge CRDT semantics are fully preserved through the encryption and relay layers. Concurrent edits merge correctly. Delivery order doesn't affect the outcome. Replay is safe. Snapshots can bootstrap new clients.
...
| Replay safety        | **Validated** | Test 3.6: Automerge internally deduplicates changes; applying the same change twice is a no-op                                                                              |
...
| File                | Purpose                                                | Lines |
| ------------------- | ------------------------------------------------------ | ----- |
| `types.ts`          | Type definitions for envelopes, keys, and domain types | 42    |
| `encrypted-sync.ts` | Encrypt/decrypt/sign/verify for changes and snapshots  | 127   |
| `relay.ts`          | In-memory append-only ciphertext relay                 | 56    |
| `sync-session.ts`   | Client session wrapping Automerge + encryption         | 106   |
| `index.ts`          | Public API re-exports                                  | 23    |

Tests are in `packages/sync/src/__tests__/`:

| File                     | Tests | Coverage                         |
| ------------------------ | ----- | -------------------------------- |
| `encrypted-sync.test.ts` | 9     | Encryption primitives            |
| `relay.test.ts`          | 6     | Relay storage and zero-knowledge |
| `sync-session.test.ts`   | 9     | Full integration scenarios       |
