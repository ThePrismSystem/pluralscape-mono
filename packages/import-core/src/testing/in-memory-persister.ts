 ...
 *    fake must mirror that behaviour so engine tests can assert on skip counts
 *    without hitting I/O.
 *
...
export interface ThrowOnSpec {
   ...
   * classifies the thrown value and reacts accordingly.
   */
...
function storageKey(entityType: ImportCollectionType, sourceEntityId: string): string {
  return `${entityType}:${sourceEntityId}`;
}

function deterministicId(entityType: ImportCollectionType, sourceEntityId: string): string {
  ...
  return {
    ...
    snapshot(): InMemoryPersisterSnapshot {
      ...
      for (const [key, value] of store.entries()) {
        ...
        const entityType = key.slice(0, separatorIndex) as ImportCollectionType;
