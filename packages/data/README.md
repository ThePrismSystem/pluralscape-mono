...
Every entity the app works with (members, fronting sessions, channels, notes, polls, innerworld entities, and more) has a corresponding set of crypto transforms in this package. The transforms decrypt ciphertext fields from API responses and encrypt plaintext fields before writes, using the XChaCha20-Poly1305 primitives from `@pluralscape/crypto`. Sensitive fields never leave the client in plaintext.
...
Every `decrypt*` function passes the decrypted blob through `<Entity>EncryptedInputSchema.parse()` from `@pluralscape/validation`. Missing fields, wrong types, or non-object payloads throw a `ZodError`, so ciphertext corruption, key mismatch, or server tampering surfaces as an error rather than silent data loss. Encrypt/decrypt pairs round-trip; for example, `encryptBucketInput` followed by `decryptPrivacyBucket` reproduces the original plaintext. Base64 encode/decode inside `decode-blob.ts` is backed by Node's `Buffer` API for linear-time conversion.
