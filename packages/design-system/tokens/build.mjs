...
const spacingScale = sortedObj(
  Object.entries(colors.scale ?? {}).concat(Object.entries(spacing.scale ?? {}))
    ...
    Object.entries(typography.scale?.default ?? {})
      ...
      .map(([k, v]) => {
        if (v && typeof v === 'object') {
          return [k, sortedObj(Object.entries(v).filter(([ik]) => ik !== '_doc').map(([ik, iv]) => [ik, primitiveVal(iv)]))];
        }
...
const hitThemes = {
  ...
  'high-contrast': sortedObj(Object.entries({ ...hitOut, ...spacing.hit?.['high-contrast'] ?? {} })),
...
const durThemes = {
  default: defaultDur,
  static: sortedObj(Object.entries({ ...defaultDur, ...Object.fromEntries(Object.entries(motion.duration?.static ?? {}).filter(([k]) => k !== '_doc')) })),
  'reduced-motion': sortedObj(Object.entries({ ...defaultDur, ...Object.fromEntries(Object.entries(motion.duration?.['reduced-motion'] ?? {}).filter(([k]) => k !== '_doc')) })),
  'high-contrast': sortedObj(Object.entries({ ...defaultDur, ...Object.fromEntries(Object.entries(motion.duration?.['high-contrast'] ?? {}).filter(([k]) => k !== '_doc')) })),
  littles: defaultDur,
};
...
const glowThemes = {
  ...
  'high-contrast': sortedObj(Object.entries({ ...defaultGlow, ...elevation.glow?.['high-contrast'] ?? {} })),
  littles: defaultGlow,
};
...
function buildTypoScale(modeOverrides) {
  ...
  for (const [k, v] of Object.entries(modeOverrides)) {
    if (k === '_doc') continue;
    if (out[k]) {
      out[k] = sortedObj(Object.entries({ ...out[k], ...Object.fromEntries(Object.entries(v).filter(([ik]) => ik !== '_doc')) }));
    }
  }
  return sortedObj(Object.entries(out));
}
